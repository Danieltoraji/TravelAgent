...
# 非正餐 venue 名称关键词（2026-09-15，用户南京实测：午餐/晚餐选中咖啡店、
# 蜜雪冰城——food 附近搜索的 POI 无正餐类目过滤，饮品/甜品/便利店与正餐同池，
# 就近策略下景点旁的奶茶店天然最近）。名称命中即从**选餐候选**过滤（Resolver
# 层放宽兜底：全黑名单时保留原池保供）。注意只匹配名称，POI type 过滤仍走
# _is_non_restaurant（两道独立防线）；「茶餐厅」（港式正餐）不含"茶饮/奶茶"
# 字样不受误伤。
_NON_MEAL_NAME_KEYWORDS = (
    "咖啡", "奶茶", "茶饮", "甜品", "甜点", "蛋糕", "面包", "烘焙", "糕点",
    "冰淇淋", "冰激凌", "糖水", "便利店", "超市", "书报", "鲜花",
    "蜜雪冰城", "瑞幸", "星巴克", "喜茶", "奈雪", "茶百道", "书亦",
    "沪上阿姨", "贡茶", "CoCo", "都可", "柠季", "益禾堂", "古茗",
    # 2026-09-15 南京复验补充：快餐盒饭/馒头包子铺同样不适合行程推荐
    "盒饭", "快餐", "食堂", "外卖", "小卖部", "馒头",
    "包子铺", "包子店",
)


def is_non_meal_name(name: Any) -> bool:
    """餐厅名含非正餐品类关键词（咖啡/奶茶/甜品/便利店等）→ True。

    供 ``RestaurantResolver`` 选餐候选过滤（放宽兜底在 Resolver 层：全命中
    黑名单时保留原池保供）。仅匹配名称——真源 POI 的 type 字段过滤仍由
    ``_is_non_restaurant`` 承担（两道防线互不替代）。
    """
    text = _as_str(name)
    return any(keyword in text for keyword in _NON_MEAL_NAME_KEYWORDS)
