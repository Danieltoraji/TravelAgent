...
_SUFFIX_PATTERNS = (
    # 「小区」须排在「区」前——lazy 组优先最短前缀，「…兴华园小区」剥「小区」
    # 而非把「区」单剥成「…兴华园小」（9.4 详细地址实测发现的残留瑕疵）
    re.compile(r"^(.*?)(?:小区|省|市|自治区|特别行政区|自治州|自治县|区|县|镇|乡|盟|旗)$"),
)
...
_REGIONS: Dict[str, Tuple[str, ...]] = {
    ...
    # 地名映射方案 b（2026-09-16 拍板）：区域名 → 城市列表作为检测依据与
    # expand_region 能力数据；「东北→主城映射」由 LLM 地址归一层
    # （call_llm.address_city_resolver）结合偏好选代表城市。
    "东北": ("哈尔滨", "长春", "沈阳", "大连"),
    # 省级区名（2026-09-18 用户实测天津→新疆）：address_city_resolver 对
    # 省级名判「范围过大无法判断」→ 走多城管线承接（用户实测编排器 LLM
    # 本就规划了乌鲁木齐→阿勒泰的正确玩法，管道此前接不住）。
    "新疆": ("乌鲁木齐", "阿勒泰", "伊犁", "吐鲁番", "喀什"),
}
